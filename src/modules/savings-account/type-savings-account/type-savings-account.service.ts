...
export class TypeSavingsAccountService {
  ...
  async create(dto: CreateTypeSavingsAccountDto): Promise<TypeSavingsAccount> {
    ...
    if (dto.interest_year_savings_account === undefined) {
      dto.interest_year_savings_account = 0.0; // Valeur par défaut
    }
  ...
  calculerSoldeMinimumDepot(produit) {
    ...
    const montantBloque = produit.minimum_blocking_duration > 0 ? 0.0 : 0.0;

    // 2. Calcul des frais récurrents (minimum balance + frais mensuels)
    const fraisRecurrents =
      (parseFloat(produit.minimum_balance) || 0.0) + (0 || 0.0);
    ...
    let total = montantBloque + fraisRecurrents + fraisUniques;
