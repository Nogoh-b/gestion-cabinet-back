...
export class TransactionSavingsAccountService {
  constructor(
   ...
   async perform_transaction(
    ...
    const dayBeforeWithdraw = (dto as CreateTransactionSavingsAccountDto).day_before_withdraw ?? 0;

    console.log('dayBeforeWithdraw', (dto as CreateTransactionSavingsAccountDto).day_before_withdraw )
    ...
    tx.date_for_withdraw = dayBeforeWithdraw > 0 ? new Date(Date.now() + dayBeforeWithdraw * 24 * 60 * 60 * 1000) : new Date();
  ...
  Promise<PaginatedResult<TransactionSavingsAccount>> {
    const qb = this.repo
      ...
      if( data.personnel_id){
        qb.andWhere('tx.personnel_id = :personnel_id', { personnel_id:data.personnel_id  });
      }
...
async findAllTrans(branch_id: number | null): Promise<TransactionSavingsAccount[]> {
  ...
  async validate(
    ...
    if(entity.status == 0){
      ...
      if(entity.provider_code == TransactionProvider.OM || entity.provider_code == TransactionProvider.MOMO ){
        const updated_sold = await this.mcotiService.callMcotiEndpoint(
              'POST',
              `epargne/bank/operator/update-sold`,{provider:'OM', isCredit: entity.origin ? 0 : 1,  amount: entity.amount + (entity.commission ?? 0),}
          );
    ...
    await this.repo.manager.transaction(async (entityManager) => {
      ...
      if (target && (isFirstTx || force)) {
        ...
        console.log('sauvegarde de la transaction de frais d\'ouverture de compte ',target.commercial_code)   
        ...
        if(target.commercial_code){
            ...
            if(comercial && comercial.savings_account){
              ...
              if((comercial.is_intern && (await this.savingsAccountService.accountCreatedByCommercial(comercial.code)).length > 10 ) || !comercial.is_intern){
                ...
                fifthTx.target = comercial?.savings_account.number_savings_account;
                fifthTx.originSavingsAccount = adminSa;
                fifthTx.origin = tx.targetSavingsAccount?.number_savings_account ?? 'SYSTEM';
        ...
        if(target.promo_code){
            ...
            fourthTx.origin = tx.targetSavingsAccount?.number_savings_account ?? 'SYSTEM';
      ...
      if (tx.commission && tx.commission > 0 && tx.channelTransaction.code == TransactionChannel.MOBILE && tx.transactionType.code != TransactionCode.INTERNAL_TRANSFER) {
        ...
        const commissionCashFinance = await this.transactionTypeService.findOneByCode(tx.provider.code === TransactionProvider.OM ? TransactionCode.COMMISSION_CASH_OM_MFINANCE : TransactionCode.COMMISSION_CASH_MOMO_MFINANCE);
  ...
  async unlockTransactionForPersonnel(personnel_id,idSa,minTxs): Promise<any> {
    ...
    if(txs[1] < minTxs){
      throw new BadRequestException(`Vous devez avoir au moins ${minTxs} transactions pour etre payé`);
    }
    this.repo.update(
        { personnel_id },
        { is_locked: false },
