...
export class GenericWriteService {
  ...
  async executePlan(writePlan: WritePlan, userId: string): Promise<WriteResult[]> {
    ...
    try {
      ...
      if (error instanceof EntityIdRequiredException) {
        ...
        throw error; // re-throw tel quel — le plan doit rester exploitable pour une reprise
      }
  ...
  private resolveReferences(
    ...
    // Valeurs spéciales : {{today}} → date du jour ISO, {{now}} → timestamp actuel ISO
    const todayDate = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
    const nowISO = new Date().toISOString();                   // YYYY-MM-DDTHH:mm:ss.sssZ
    const replaceSpecialValues = (s: string): string =>
      s
        .replace(/\{\{today\}\}/gi, todayDate)
        .replace(/\{\{now\}\}/gi, nowISO);
    ...
    for (const [key, value] of Object.entries(fields)) {
      ...
      // ✅ Remplacer {{today}} et {{now}} AVANT les références tempId
      const withSpecials = replaceSpecialValues(value);

      // ✅ Format principal : {{tempId.field}}
      if (withSpecials.includes('{{')) {
        resolved[key] = withSpecials.replace(mustacheRegex, (match, tempId, field) => {
      ...
      if (withSpecials.startsWith('$')) {
        const [tempId, fieldName] = withSpecials.slice(1).split('.');
        if (createdEntities.has(tempId)) {
          const entity = createdEntities.get(tempId);
          resolved[key] = fieldName ? entity[fieldName] : entity;
          this.logger.debug(`🔗 Référence résolue (legacy): ${withSpecials} → ${resolved[key]}`);
        } else {
          this.logger.warn(`⚠️ Référence non résolue: ${withSpecials} (tempId "${tempId}" inconnu)`);
          resolved[key] = withSpecials;
        }
        continue;
      }
